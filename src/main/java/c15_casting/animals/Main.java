...
/*
    ...
    참조 자료형 캐스팅(Reference Type Casting)
        객체 지향 프로그래밍(00P)에서 중요한 요수 중 하나입니다.

        1. 참조 자료형이란?
            자바에서의 자료형은 크게 두 가지로 구분됨.
                1) 기본 자료형(Printive Type) : int, double, cher 등
                2) 참조 자료형(Reference Type) : 객체를 참조하는 변수로 클래스나 인터페이스를 통해 생성된 객체
        ...
        5. 캐스팅의 장단점
            장점 :
                ...
                다향성 구현 : 메서드 오버라이딩과 함께 사용하면 코드 재사용성을 높일 수 있음
    ...
    instanceof 연산자 : Java에서 객체의 실제 타입을 검사하는데 사용(어떤 클래스의 객체인지를 확인하는 용도)
        -> 주로 다운 캐스팅의 안전성을 확보하기 위해서 사용함.

            형식 :
                object(객체명) instanceof ClassName(클래스 명)     > 1 + 1 생각하라고 수업했습니다.
...
//        animal1.fetch();      // 타입이 Animal로 선언되었기 때문에
                                // Animal 클래스에는 없는 fetch() 메서드를 호출할 수 없음

        System.out.println();
        System.out.println("----------- animal2 부분 시작 --------------");
        Animal animal2 = new Dog();     // 없캐스팅 2번 방법 > 얘가 정처기 등 자격증 시험에 더 자주 나옵니다.
        // A a = new B()
        animal2.makeSound();            // 'Dog'의 재정의된 makeSound() 호출됨
//        animal2.fetch();              // animal1때와 동일하기 호출 불가능함
        System.out.println();
        ...
        Dog dog2 = (Dog) animal2;   // 다운캐스팅의 경우 명시적으로 쓰여야하기 때문에
                                    ...
                                    // 이제는 클래스오 ㅏ객체 단위에서 생각할 필요가 있습니다.
        ...
        /*
            * Try:
            ...
            2 actionable tasks: 2 executed
                    ...
                    그 이유는 :
                        애초에 Animal로 만든 것을 아무런 변형없이 강제적으로 다운캐스팅을 시도했기 때문입니다.
