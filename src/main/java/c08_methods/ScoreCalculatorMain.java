...
public class ScoreCalculatorMain {
    public static void main(String[] args) {
        ...
        // ScoreCalculator를 import > 클래스명 객체명 = new 클레스명();     > constructor
        ScoreCalculator scoreCalculator = new ScoreCalculator();
        int totalSubjects = 0;
        double score1 = 0;
        double score2;
        // double totalScore = 0으로 선언 및 초기화하는 것은 필수이므로 표시해둠.
        double totalScore = 0;
        double avgScore = 0;
...
//        System.out.println("평균 점수는" + avgScore + "점 입니다. (메서드 활용");
        ...
        while (!endOfCalc) {      // endOfCalc를 추후 true로 바꾸는 조건을 통해 반복문 종료 예정
            ...
            score1 = scanner.nextDouble();
            if (score1 == -1) {
                break;      // swich문에서 배운 break문으로 : 반복문 종료할 때 쓰임
                // ㄴ곧장 반복문을 종료시키고자 할 때 사용
//                endOfCalc = true; // endOfCalc라는 변수를 true로 바꿔줌으로써 반복문 종료
                // ㄴ다음 반복문이 실행될 때 boolean 연산자의 true/false를 검증하기 때문에 그 전까지는 그대로 실행됨
            }
