...
public class UserService {
    /*
            ...
            데이터베이스 테이블 매핑 : Entity 클래스는 데이터베이스 테이블의 구조와 일치
                -> 지난 시간에 +----------------+ 이 형태로 그렸던 표과 관련있습니다.
                필드와 칼럼 매핑 : 클래스의 필드는 데이터베이스 테이블의 컬럼과 매핑됩니다. (마찬가지로 지난 시간 수업)
                ...
                캡슐화(Encapulation) : 객체의 필드와 메서들로 하나를 묶고, 일부를 외부에 감추어 은닉하는 것
                ...
                해시코드(Hashcode) : 객체의 고유한 정수 값을 반환하는 메서드
                    ...
                    3. 규약(protocol) :
                        일관성 : 같은 객체에 대해 여러 번 호출 된 hashCode() 메서드는 항상 동일한 정수값을 반환해야 함.
