...
/*
    1. Setter / Getter의 필요성
        ...
        그리고 이름 PersonMain에서 객체를 생성했다고 가정하겠습니다.
        class PersonMain {
        ...
        해당 코드를 확인했을 때 나이에 정상적이지 않은 값이 대입되어있습니다.
    ...
    2. Setter / Getter란?
        ...
        Getter
            ...
            못하도록 하고, 특정 메서드를 통해 안전하게 값을 반환힙니다.

            Person.java / PersonMain.java를 확인할 것 > 응용코드 작성

            수업 내용 확인을 위해 파일 내부를 검색하려면   -> ctrl + f
            여태까지의 수업한 전체 패키지 및 파일 확인에서 내용을 확인하려면    -> ctrl + shift + f

            해결책 : 캡슐화(encapsulation)
                클래스 내부의 데이터를 외부에서 바로 접근하지 못하게 하고,
                대신 "메서드"를 통해서 갑접적으로 접근하도록 만드는 것.
